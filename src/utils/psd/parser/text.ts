import { Justification, Layer, LayerEffectsInfo, ParagraphStyle } from "ag-psd";
import { getCommonOptions, LayerInfo } from "./common";
import { Group, IPaint, isArray, ITextAlign, Text } from "leafer-ui";
...
function parseStyledText(layer: LayerInfo, options = {}) {
  ...
  const svgContent = layer.text.styleRuns.reduce((acc, item, index) => {
    ...
    const text = textStr
      .substring(startLen, endLen)
      .replace(/([^\S\n]*)\n/g, "<br/>")
      .replace(/([^\S\n]*)\t/g, "&nbsp;");
    startLen = endLen;
    return (
      acc +
      `<span style="font-size:${fontSize}px;color: ${textUtil.getFillStr(layer)};">${text}</span>`
    );
  }, "");
...
export const textUtil = {
  ...
  getFill(layer: Layer): IPaint[] {
    if (
      ...
      return [
        { type: "solid", color: toRGBColorStr(layer.text?.style?.fillColor) },
      ];
    } else {
      // 默认黑色
...
const setTxtParagraphStyle = (paragraphStyle: ParagraphStyle, text: Text) => {
  if (paragraphStyle) {
    if (paragraphStyle.justification) {
      text.textAlign = textUtil.mapJustificationToTextAlign(
