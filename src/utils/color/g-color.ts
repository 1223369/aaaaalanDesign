...
const hsv2rgba = (states: ColorStates): tinyColor.ColorFormats.RGBA =>
  tinyColor(states).toRgb();
const hsv2hsva = (states: ColorStates): tinyColor.ColorFormats.HSVA =>
  tinyColor(states).toHsv();
const hsv2hsla = (states: ColorStates): tinyColor.ColorFormats.HSLA =>
  tinyColor(states).toHsl();
 ...
 * @returns
 */
export const getColorWithoutAlpha = (color: string) =>
  tinyColor(color).setAlpha(1).toHexString();
...
export class GColor {
  ...
  update(input: string) {
    ...
    if (gradientColors) {
      ...
      const points = object.points.map((c) =>
        genGradientPoint(c.left, c.color),
      );
      this.gradientStates = {
        colors: points,
  ...
  updateCurrentGradientColor() {
    ...
    const index = gradientColors.findIndex(
      (color) => color.id === gradientSelectedId,
    );
    const newColor = {
      ...current,
      color: this.rgba,
  ...
  static compare = (color1: string, color2: string): boolean => {
    ...
    if (isGradientColor1 && isGradientColor2) {
      const gradientColor1 = gradientColors2string(
        parseGradientString(color1) as GradientColors,
      );
      const gradientColor2 = gradientColors2string(
        parseGradientString(color2) as GradientColors,
      );
 ...
 * @description 获取对外输出的 color 描述对象
 * @param {GColor} color GColor 实例对象
 * @returns {ColorObject | null} 规范化的 ColorObject 对象
 */
export const getColorObject = (color: GColor): ColorObject | null => {
  if (!color) {
    return null;
  }
  ...
  return colorObject as ColorObject;
};
