...
export const useFontStore = defineStore("font", () => {
  ...
  async function initFonts() {
    let list = [];
    localStorage.getItem(FONT_VERSION_KEY) !== "1" &&
      localStorage.removeItem(FONT_KEY);
    const localFonts: any = localStorage.getItem(FONT_KEY)
   ...
   */
  async function extractTemplateFonts(
    jsonData: any,
    load = false,
  ): Promise<TemplateFontFamily> {
