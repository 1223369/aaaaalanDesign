  ...
  <a-space>
    ...
    >
      ...
      <template #content v-if="button.doption">
        <a-doption
          v-for="doption in button.doption"
          :key="doption.key"
          :value="doption"
        >
          <template #icon>
...
// import {useEditor} from '@/views/Edito'
...
function getFilteredDataByKey(key: string) {
  ...
  editToolList.value.forEach((item, index) => {
    ...
    if (item.doption) {
      const subItemIndex = item.doption.findIndex(
...
const onSelect = (index: number, value: any) => {
  editToolList.value[index] = {
    ...editToolList.value[index],
    ...(value as EditToolListItem),
  };
