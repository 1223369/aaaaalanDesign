...
import type { DialogReturn } from "@/components/dialog/interface";
...
const openDialog = (
  accessor: ServicesAccessor,
  {
    ...
    initialColor,
    ...props
  }: ColorPickerOption & Partial<Props>,
) => {
  ...
  const colorArr =
    <[]>(
      (object && attr
        ? (object.proxyData?.[attr] ?? object[attr])
        : [initialColor])
    ) ?? [];
  ...
  // 纯色
    ...
    const color = new GColor(colorStr);
    ...
    body: () =>
      h(_ColorPicker, {
        onChange(data: any) {
          ...
          const colorArr = object.proxyData?.[attr] ?? object[attr] ?? [];
