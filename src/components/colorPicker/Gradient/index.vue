          ...
          >
            <template #label>
              <div style="text-align: right; padding-right: 12px">
                <icon-mosaic style="margin-right: 2px" />透明度
              </div>
            </template>
            <template #suffix>
              <div>%</div>
...
const fit = computed({
  ...
  set(value) {
    if (typeUtil.isPattern(object.value.modelValue[state.index]) && value) {
      ...
      const newElement = replaceElementToNewArr(
...
const newOpacity = computed({
  get() {
    if (typeUtil.isPattern(object.value.modelValue[state.index])) {
      ...
      if (object.value.modelValue[state.index].opacity !== undefined) {
        opacity = NP.round(
          object.value.modelValue[state.index].opacity * 100,
          2,
        );
      }
      return opacity;
  ...
  set(value) {
    if (
      typeUtil.isPattern(object.value.modelValue[state.index]) &&
      value !== undefined
    ) {
      // 这里使用新数组，因为leafer是浅监听的 修改数组值无法监听到并重新渲染
      const elementVal: any = object.value.modelValue[state.index];
...
onMounted(() => {
  const color = new Color(
    new RGBA(state.colorRed, state.colorGreen, state.colorBlue, 1),
  );
  const { h, s, v } = color.hsva;
